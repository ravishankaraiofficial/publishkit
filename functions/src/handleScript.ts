...
export const generateScript = functions
  ...
  .https.onCall(async (data, context) => {
    try {
      ...
      // Effective tone: profile-level tone preset takes precedence over per-call default.
      const effectiveTone = profileTone || tone;

      // Build creator-voice preamble (only include fields the user filled).
      ...
      // SECURITY INVARIANT: keep the language directive and JSON structure rules
      // AFTER all user-controlled profile text. Positional precedence makes
      // prompt-injection ("ignore previous instructions...") much harder to land.
      const prompt = `You are an expert YouTube scriptwriter writing in the creator's own voice.
