...
const EXTENSION_ID = process.env.BROWSE_EXTENSION_ID || '';
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      if (url.pathname === '/health') {
        ...
        return new Response(JSON.stringify({
          ...
          // However, to prevent token leakage via DNS rebinding or malicious extensions,
          // strictly validate against the configured BROWSE_EXTENSION_ID.
          ...(EXTENSION_ID && req.headers.get('origin') === `chrome-extension://${EXTENSION_ID}`
              ? { token: authToken } : {}),
