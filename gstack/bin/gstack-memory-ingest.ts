...
import { join, basename, dirname, sep, resolve } from "path";
...
function writeStaged(prepared: PreparedPage[], stagingDir: string): StagingResult {
  ...
  let written = 0;
  const stagingDirAbs = resolve(stagingDir);
  for (const p of prepared) {
    const relPath = `${p.slug}.md`;
    const absPath = resolve(stagingDir, relPath);
    if (!absPath.startsWith(stagingDirAbs + sep)) {
      errors.push({ slug: p.slug, error: "Path traversal detected" });
      continue;
    }
    try {
